 ...
 * the state actually is. With category colour gone, these four states are the
 * only chromatic distinction left beside severity itself.
 *
 ...
 *
 * `non-verifiable` takes the dashed warning the panel already uses to declare the
 ...
 * every card and `Majeur` is a solid warning fill, so two filled warning chips
 * side by side would read as one alarm louder than `Douteuse`. Filled means a
 * verdict was reached; the outline means nothing was established either way.
 *
...
export const VERIFICATION_STYLES: Record<VerificationState, VerificationStyle> = {
  verifiee: {
    ...
    className:
      'bg-severity-positive/15 text-severity-positive-ink border-severity-positive/50',
  },
  'non-sourcee': {
    ...
    className: 'bg-panel-muted text-muted border-line',
  },
  douteuse: {
    label: 'Douteuse',
    hint: 'Les sources consultées, listées ci-dessous, mettent en doute ce point de l’article.',
    className:
      'bg-severity-critical/15 text-severity-critical-ink border-severity-critical/50',
  },
  'non-verifiable': {
    ...
    className:
      'bg-panel text-severity-warning-ink border-severity-warning/50 border-dashed',
  },
};
...
export const UNRECOGNISED_STYLE: VerificationStyle = {
  ...
  className: 'bg-panel text-faint border-line border-dotted',
};
